...
path_in = r'/data/SF/sf/hd8data/B03'
...
YearMonthDay=int(filename.split('/')[-1].split('_')[2][:])
path_tj='/data/himawari_cloud_top/out/bin/'+str(YearMonthDay)+'/China/Cloudtop'
isExists = os.path.exists(path_tj)
if not isExists:
		# 如果不存在则创建目录
		# 创建目录操作函数
		os.makedirs(path_tj)

path_tj_1='/data/himawari_cloud_top/out/figure/'+str(YearMonthDay)+'/China/Cloudtop'
isExists = os.path.exists(path_tj_1)
if not isExists:
		# 如果不存在则创建目录
		# 创建目录操作函数
		os.makedirs(path_tj_1)
...
cloud_clfed = np.power(cloud_clfed, 0.975)
...
fig.savefig(path_tj_1 + '/' + filename_out + '.jpg')

with open(path_tj + '/' + filename_out + '.bin','wb') as f:
    ss = 'Bottom={},Coordination=GEO,DataType=6,Day={},Dimension=1,Forecast=000,Height={},Hour={},Invalid=0.0000,Left={},LevelUnit=,LevelValue=,Levels=1,Machine=LittleEnding,Minute={},Month={},NorthtoSouth=True,Offset=0,ProductId=CLOUDTOP,Ratio=1.0000,ResolutionCell=0,Resolution_x={},Resolution_y={},Right={},Secod=00,SiteCode=SAT,Storage=CFORMAT,Top={},Width={},Year={}'.format('%.4f'%EndLat,
                                                                                   '%02d'%Day,
                                                                                   '%04d'%YNumGrids,
                                                                                   '%02d'%Hour,
                                                                                   '%.4f'%StartLon,
